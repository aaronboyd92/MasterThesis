...
public abstract class RelaxedGraphHeuristic extends AbstractHeuristic {
    ...
    protected final int CriticalPath(){
        int critical = Integer.MIN_VALUE;
  ...
  BitVector n_goal = super.getGoal().getNegative();
        ...
        if (goalCardinality <= k){
         for (int p = p_goal.nextSetBit(0); p >= 0; p = p_goal.nextSetBit(p + 1)) {
             final int gl = this.pPropLevel[p];
            if (gl > goalCardinality) {
                critical = gl; 
            }
        }
 
        for(int p = n_goal.nextSetBit(0); p >= 0; p = n_goal.nextSetBit(p + 1)) {
            final int gl = this.nPropLevel[p];
            if (gl > goalCardinality) {
                critical = gl;
            }
        }
            for(BitOp op : this.getOperators()){
           BitExp effects1 = op.getCondEffects().get(0).getEffects();
        BitVector positiveEffect = effects1.getPositive();
        BitVector negativeEffect = effects1.getNegative();
                for (int p = negativeEffect.nextSetBit(0); p >= 0; p = negativeEffect.nextSetBit(p + 1)) {
                 this.effects[p].getNegative();
                }
                for (int p = positiveEffect.nextSetBit(0); p >= 0; p = positiveEffect.nextSetBit(p + 1)) {
                    this.effects[p].getPositive();
                }
        ...
        int [] combinations = new int [goalCardinality];
        ...
        //if the subset size equals k, return subset
        if(sub.size() >= k) {
        critical = sub.size();
        this.getMaxValue();
