...
private static final Heuristic.Type DEFAULT_HEURISTIC = Heuristic.Type.FAST_FORWARD;
    ...
    public String Convert(CodedProblem other, Plan plan) {
      BitState state = new BitState(init);
      BitState state1 = new BitState();
        final Heuristic heuristic = HeuristicToolKit.createHeuristic(DEFAULT_HEURISTIC, other);
      int step = heuristic.estimate(state, goal);
      BitState goalState = new BitState(other.getGoal());
      int initSize = init.cardinality();
      int steps = 0; 
      //System.out.println(plan.actions());
      
       //1. Gets initial state size (variables for sat problem)
         System.out.println("p cnf "+ initSize +" "+ step);   
             String sat = null;
        System.out.print("");
        ...
        System.out.print("\n");
        for(BitOp op : this.operators){            
      //3. POSITIVE PRECONDITIONS AND POSITIVE EFFECTS
             for (int i = steps; 0 <= i; i--) {
        BitExp precon = op.getPreconditions(); 
        //System.out.println(plan.actions().toString());
        BitVector precond = new BitVector(precon.getNegative());
       System.out.print("");
  for (int p = precond.nextSetBit(0); p >= 0; p = precond.nextSetBit(p + 1)) {
         System.out.print("-"+p+" "); 
        }
  precond = precon.getPositive();
    for (int p = precond.nextSetBit(0); p >= 0; p = precond.nextSetBit(p + 1)) {
         System.out.print(p+" "); 
        }
        System.out.print("\n");
                      sat = other.toString(precon);
        // System.out.println(sat);
        BitExp effects = op.getCondEffects().get(steps).getEffects();  
        BitVector effect = new BitVector(effects.getPositive());
        System.out.print("");
      for (int p = effect.nextSetBit(0); p >= 0; p = effect.nextSetBit(p + 1)) {      
           System.out.print(p+" ");
      }
        effect = effects.getNegative();
     for (int p = effect.nextSetBit(0); p >= 0; p = effect.nextSetBit(p + 1)) {      
           System.out.print("-"+p+" ");
      }     
            System.out.print("\n");  
         ...
         System.out.print("");
    for (int p = state1.nextSetBit(0); p >= 0; p = state1.nextSetBit(p + 1)) {
        System.out.print(p+" ");
    }
          System.out.print("\n"); 
             } 
